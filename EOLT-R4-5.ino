...
void setup() {
  ...
  */
  ...
  // ====================Software Version====================
  String softwareVersion = "0.0.0.4.5"; // update this as the program changes
  String softwareDate = "12/24/2018"; // update this as the program changes
    ...
    if (editingRow >= 3) {
      ...
      while (buttonStateUp == LOW) {
        ...
        for (int j = 0; j <  column9numberHeads * column8numberHallsPerHead; j++) {
          Serial.print(uutVerbose[j]);  //dump uutVerbose array to SD card
          Serial.print(", ");  // this also leaves a comma at the end of the line
        }
        Serial.println();
      }
...
void rightButton() {
  if (buttonStateRight) {
    ...
    switch (editingRow) {
      ...
      case 3:// this is the streaming mode
